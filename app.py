...
selected_position = st.sidebar.multiselect('Position', sorted_position_unique, sorted_position_unique)
# Sidebar - Title selection
sorted_title_unique = sorted( df_3['title'].unique() )
selected_title = st.sidebar.multiselect('Title', sorted_title_unique, sorted_title_unique)
# Filtering data
df_selected = df_3[ (df_3['position'].isin(selected_position)) & (df_3['title'].isin(selected_title)) ]
st.title('Job offers:')
